...
### Accepted dissent (deepseek, "Member B"), and it changed the build
...
### Auth: the council genuinely split, and this is the weakest call here
...
### Grievance history: an append-only, hash-chained event log
...
- **Lighthouse, then market.** The alma mater is the free reference deployment, not the
  ...
  flagged can bypass the 9 to 18 month government tender cycle that would outlast any runway.
- **Price: ₹75,000 scoped paid pilot → ₹1.5 to 3L per year** for a mid-size private college.
